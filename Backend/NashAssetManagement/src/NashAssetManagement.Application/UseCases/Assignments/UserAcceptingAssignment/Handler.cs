...
namespace NashAssetManagement.Application.UseCases.Assignments.UserAcceptingAssignment
{
        ...
        : IRequestHandler<Request, ErrorOr<Updated>>
    {
        public async Task<ErrorOr<Updated>> Handle(Request request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                assignment.Accept(dateTimeProvider.UtcNow);
                await uow.SaveChangesAsync(cancellationToken);
                return Result.Updated;
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "Failed to accept assignment with Id '{AssignmentId}', request initialized by user '{UserId}'", assignmentId, userId);
